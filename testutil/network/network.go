...
func DefaultConfig() Config {
	...
	chainID := fmt.Sprintf("nexa_%d-1", tmrand.Int63n(9999999999999)+1)
	return Config{
		Codec:             encCfg.Codec,
		...
		NumValidators:     4,
		BondDenom:         "aNEXB",
		MinGasPrices:      fmt.Sprintf("0.000006%s", nexatypes.AttoNexa),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, nexatypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, nexatypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, nexatypes.PowerReduction),
...
// NewAppConstructor returns a new Nexa AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !nexatypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "nexad")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "nexacli")
		...
		customAppTemplate, _ := config.AppConfig(nexatypes.AttoNexa)
