...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the aNEXB amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	...
	res, err := DeliverEthTx(nexaApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, nexaApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := nexaApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := nexaApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := nexaApp.EvmKeeper.GetNonce(ctx, from)
