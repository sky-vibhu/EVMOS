...
func PrepareEIP712CosmosTx(
	...
	accNumber := appNexa.AccountKeeper.GetAccount(ctx, from).GetAccountNumber()

	nonce, err := appNexa.AccountKeeper.GetSequence(ctx, from)
	if err != nil {
		return nil, err
	}
...
func createTypedData(args typedDataArgs, useLegacy bool) (apitypes.TypedData, error) {
	if useLegacy {
		...
		nexaCodec := codec.NewProtoCodec(registry)
