...
func (s *PrecompileTestSuite) CreateTestClawbackVestingAccount(funder, vestingAddr common.Address) {
	...
	// err = nexautil.FundAccount(s.ctx, s.app.BankKeeper, vestingAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, sdk.NewInt(100))))
	// s.Require().NoError(err)
...
func (s *PrecompileTestSuite) NextBlock() {
	var err error
	s.ctx, err = nexautil.CommitAndCreateNewCtx(s.ctx, s.app, time.Second, nil)
