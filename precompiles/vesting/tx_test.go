...
var (
	...
	toAddr           = nexautiltx.GenerateAddress()
	funderAddr       = nexautiltx.GenerateAddress()
	diffFunderAddr   = nexautiltx.GenerateAddress()
...
func (s *PrecompileTestSuite) TestCreateClawbackVestingAccount() {
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() []interface{} {
				differentAddr := nexautiltx.GenerateAddress()
				return []interface{}{
					funderAddr,
					differentAddr,
					false,
	...
	testCases := []struct {
		...
		},
		{
			...
			func() []interface{} {
				...
				// err = nexautil.FundAccount(s.ctx, s.app.BankKeeper, toAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, sdk.NewInt(100))))
				return []interface{}{
					s.address,
					toAddr,
					uint64(time.Now().Unix()),
	...
	testCases := []struct {
		...
		},
		{
			...
			func(data []byte) {
				...
				_, ok := account.(nexatypes.EthAccountI)
				s.Require().True(ok)
			},
			false,
			"",
