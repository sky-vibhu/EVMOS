...
import (
	...
	nexacontracts "github.com/nexablock/nexablock/contracts"
	nexatesting "github.com/nexablock/nexablock/ibc/testing"
	"github.com/nexablock/nexablock/precompiles/authorization"
	cmn "github.com/nexablock/nexablock/precompiles/common"
	"github.com/nexablock/nexablock/precompiles/ics20"
	"github.com/nexablock/nexablock/precompiles/testutil"
	"github.com/nexablock/nexablock/precompiles/testutil/contracts"
...
var (
	...
	// array of allocations with only one allocation for 'aNEXB' coin
	defaultSingleAlloc []ics20.Allocation
	// array of allocations with only two allocation for 'aNEXB' and 'uatom' coins
...
var _ = Describe("IBCTransfer Precompile", func() {
	...
	Describe("Execute increase allowance transaction", func() {
		...
		It("should increase allowance by 1 NEXA", func() {
			s.setTransferApproval(defaultCallArgs, s.differentAddr, defaultSingleAlloc)
	...
	Describe("Execute transfer transaction", func() {
		...
		Context("without authorization", func() {
			...
			It("should succeed in transfer transaction but should timeout and refund sender", func() {
				...
				timeoutMsgFee := math.NewInt(nexatesting.DefaultFeeAmt * 2)
			...
			It("should not transfer other account's balance", func() {
				...
				// err := nexautil.FundAccountWithBaseDenom(s.chainA.GetContext(), s.app.BankKeeper, s.differentAddr.Bytes(), amt)
				// Expect(err).To(BeNil())
			...
			Context("without authorization", func() {
				It("should transfer registered ERC20s", func() {
					...
					balance := s.app.Erc20Keeper.BalanceOf(
						s.chainA.GetContext(),
						nexacontracts.ERC20MinterBurnerDecimalsContract.ABI,
						erc20Addr,
						s.address,
					)
				...
				It("should not transfer other account's balance", func() {
					...
					defaultERC20CallArgs := contracts.CallArgs{
						...
						ContractABI:  nexacontracts.ERC20MinterBurnerDecimalsContract.ABI,
						PrivKey:      s.privKey,
						GasPrice:     gasPrice,
					}
					...
					mintCheck := testutil.LogCheckArgs{
						ABIEvents: nexacontracts.ERC20MinterBurnerDecimalsContract.ABI.Events,
...
var _ = Describe("Calling ICS20 precompile from another contract", func() {
	...
	Context("transfer method", func() {
		...
		Context("'aNEXB' coin", func() {
			Context("with authorization", func() {
				BeforeEach(func() {
					// set approval to transfer 'aNEXB'
					s.setTransferApprovalForContract(defaultApproveArgs)
				})
		...
		Context("IBC coin", func() {
			...
			Context("without authorization", func() {
				It("should not transfer IBC coin", func() {
					// initialNexaBalance := s.app.BankKeeper.GetBalance(s.chainA.GetContext(), s.address.Bytes(), s.bondDenom)
					...
					// fees := sdk.NewIntFromBigInt(gasPrice).MulRaw(res.GasUsed)
					// finalBalance := s.app.BankKeeper.GetBalance(s.chainA.GetContext(), s.address.Bytes(), s.bondDenom)
					// Expect(finalBalance.Amount).To(Equal(initialNexaBalance.Amount.Sub(fees)))
	...
	Context("tranfer a contract's funds", func() {
		...
		Context("transfer 'aNEXB", func() {
			// var defaultTransfernexaArgs contracts.CallArgs
			BeforeEach(func() {
				// send some funds to the contract from which the funds will be sent
				// err = nexautil.FundAccountWithBaseDenom(s.chainA.GetContext(), s.app.BankKeeper, contractAddr.Bytes(), amt)
				Expect(err).To(BeNil())
				senderInitialBalance := s.app.BankKeeper.GetBalance(s.chainA.GetContext(), contractAddr.Bytes(), s.bondDenom)
				Expect(senderInitialBalance.Amount).To(Equal(math.NewInt(amt)))

				// defaultTransfernexaArgs = defaultTransferArgs.WithArgs(
				// 	s.transferPath.EndpointA.ChannelConfig.PortID,
				// 	s.transferPath.EndpointA.ChannelID,
				// 	s.bondDenom,
				// 	defaultCmnCoins[0].Amount,
			...
			Context("without authorization", func() {
				It("should not transfer funds", func() {
					...
					// _, _, err := contracts.CallContractAndCheckLogs(s.chainA.GetContext(), s.app, defaultTransferNexaArgs, execRevertedCheck)
					Expect(err).To(HaveOccurred(), "error while calling the smart contract: %v", err)
			...
			Context("with authorization", func() {
				...
				It("should transfer funds", func() {
					...
					// res, ethRes, err := contracts.CallContractAndCheckLogs(s.chainA.GetContext(), s.app, defaultTransferNexaArgs, logCheckArgs)
					// Expect(err).To(BeNil(), "error while calling the smart contract: %v", err)
