...
func (s *PrecompileTestSuite) NewTestChainWithValSet(coord *ibctesting.Coordinator, valSet *tmtypes.ValidatorSet, signers map[string]tmtypes.PrivValidator) *ibctesting.TestChain {
	...
	s.differentAddr = nexautiltx.GenerateAddress()
...
func (s *PrecompileTestSuite) NewTransferAuthorizationWithAllocations(ctx sdk.Context, app *nexaapp.Nexa, grantee, granter common.Address, allocations []transfertypes.Allocation) error {
...
func (s *PrecompileTestSuite) NewTransferAuthorization(ctx sdk.Context, app *nexaapp.Nexa, grantee, granter common.Address, path *ibctesting.Path, coins sdk.Coins, allowList []string) error {
...
func (s *PrecompileTestSuite) setupIBCTest() {
	...
	s.app = s.chainA.App.(*nexaapp.Nexa)
	...
	s.transferPath = nexaibc.NewTransferPath(s.chainA, s.chainB) // clientID, connectionID, channelID empty
	nexaibc.SetupPath(s.coordinator, s.transferPath)             // clientID, connectionID, channelID filled
...
// TODO upstream this change to nexa (adding gasPrice)
// DeployContract deploys a contract with the provided private key,
// compiled contract data and constructor arguments
func DeployContract(
	ctx sdk.Context,
	nexaApp *nexaapp.Nexa,
	...
	chainID := nexaApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := nexaApp.EvmKeeper.GetNonce(ctx, from)
	...
	gas, err := nexautiltx.GasLimit(ctx, from, data, queryClientEvm)
	if err != nil {
		return common.Address{}, err
	}
	...
	msgEthereumTx := evmtypes.NewTx(&evmtypes.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: nexaApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := nexautil.DeliverEthTx(nexaApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := nexautil.CheckEthTxResponse(res, nexaApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func (s *PrecompileTestSuite) DeployERC20Contract(chain *ibctesting.TestChain, name, symbol string, decimals uint8) (common.Address, error) {
	addr, err := DeployContract(
		...
		gasPrice,
		s.queryClientEVM,
		nexacontracts.ERC20MinterBurnerDecimalsContract,
		name,
		symbol,
		decimals,
	)
