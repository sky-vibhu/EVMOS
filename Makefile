...
HTTPS_GIT := https:// github.com/nexablock/nexablock.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=nexa \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(NEXA_BINARY) \
...
build-docker:
	...
	mkdir -p ./build/.nexad
	echo '#!/usr/bin/env bash' > ./build/nexad
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/nexad
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/nexad
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.nexad:/home/nexa/.nexad $$IMAGE_NAME nexad "$$@"' >> ./build/nexad
	chmod +x ./build/nexad
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/nexa/nexa"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(NEXA_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/nexa:Z nexa/node "./nexad testnet init-files --v 4 -o /nexa --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\nexad:/nexa\Z nexa/node "./nexad tendermint unsafe-reset-all --home=/nexa"
	@docker run --rm -v $(CURDIR)\build\node1\nexad:/nexa\Z nexa/node "./nexad tendermint unsafe-reset-all --home=/nexa"
	@docker run --rm -v $(CURDIR)\build\node2\nexad:/nexa\Z nexa/node "./nexad tendermint unsafe-reset-all --home=/nexa"
	@docker run --rm -v $(CURDIR)\build\node3\nexad:/nexa\Z nexa/node "./nexad tendermint unsafe-reset-all --home=/nexa"
else
	@docker run --rm -v $(CURDIR)/build/node0/nexad:/nexa:Z nexa/node "./nexad tendermint unsafe-reset-all --home=/nexa"
	@docker run --rm -v $(CURDIR)/build/node1/nexad:/nexa:Z nexa/node "./nexad tendermint unsafe-reset-all --home=/nexa"
	@docker run --rm -v $(CURDIR)/build/node2/nexad:/nexa:Z nexa/node "./nexad tendermint unsafe-reset-all --home=/nexa"
	@docker run --rm -v $(CURDIR)/build/node3/nexad:/nexa:Z nexa/node "./nexad tendermint unsafe-reset-all --home=/nexa"
endif
...
PACKAGE_NAME:=github.com/nexa/nexa
