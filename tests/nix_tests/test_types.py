...
def test_block(nexa, geth):
    # Perform block queries (eth_getBlockByNumber & eth_getBlockByHash)
    # on nexa and geth nodes, and compare the responses
    # Case 1: block without transactions
    get_blocks(nexa, geth, False)
    # Case 2: block with transactions
    get_blocks(nexa, geth, True)


def get_blocks(nexa_rpc_ws, geth, with_transactions):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    ...
    nexa_blk = w3.eth.get_block(1)
    # Get existing block, no transactions
    eth_rsp = eth_rpc.make_request(
        "eth_getBlockByHash", [nexa_blk["hash"].hex(), with_transactions]
    )
...
def test_accounts(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_syncing(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_coinbase(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_max_priority_fee(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_gas_price(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_block_number(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_balance(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_storage_at(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_proof(nexa_rpc_ws, geth):
    # on nexa the proof query will fail for block numbers <= 2
...
def test_get_code(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_block_transaction_count(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_transaction(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_transaction_receipt(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_fee_history(nexa_rpc_ws, geth):
    w3: Web3 = nexa_rpc_ws.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_estimate_gas(nexa_rpc_ws, geth):
...
def test_incomplete_send_transaction(nexa_rpc_ws, geth):
