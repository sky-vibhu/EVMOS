...
def grpc_eth_call(port: int, args: dict, chain_id=None, proposer_address=None):
    ...
    return requests.get(f"http://localhost:{port}/nexa/evm/v1/eth_call", params).json()


def test_grpc_mode(custom_nexa):
    """
    ...
    api_port = ports.api_port(custom_nexa.base_port(1))
    ...
    with (custom_nexa.base_dir / "node1.log").open("a") as logfile:
        ...
        try:
            ...
            grpc_port = ports.grpc_port(custom_nexa.base_port(1))
            ...
            addr = custom_nexa.cosmos_cli(0).consensus_address()
