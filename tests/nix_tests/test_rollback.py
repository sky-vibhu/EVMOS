...
from .network import setup_custom_nexa
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:nexa_9000-1-node{i}":
            ini[section].update(
                {
...
def custom_nexa(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-nexad.nix",
    ]
    print(*cmd)
    broken_binary = Path(subprocess.check_output(cmd).strip().decode()) / "bin/nexad"
    print(broken_binary)

    # init with genesis binary
    yield from setup_custom_nexa(
        path,
        26300,
...
def test_rollback(custom_nexa):
    ...
    print("stop node1")
    supervisorctl(custom_nexa.base_dir / "../tasks.ini", "stop", "nexa_9000-1-node1")

    print("do rollback on node1")
    cli1.rollback()

    print("switch to normal binary")
    update_node_cmd(custom_nexa.base_dir, "nexad", 1)
    supervisorctl(custom_nexa.base_dir / "../tasks.ini", "update")
    wait_for_port(target_port)
