...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network nexa', 'run all tests using nexa network')
    .example(
      '$0 --network nexa --allowTests=test1,test2',
      'run only test1 and test2 using nexa network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|nexa')
    .describe(
      'batch',
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print nexad output, default false').argv
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'nexa' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or nexa')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-nexa']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-nexa'
...
function setupNetwork ({ runConfig, timeout }) {
  if (runConfig.network !== 'nexa') {
    // no need to start ganache. Truffle will start it
    return
  }

  // Spawn the nexa process

  const spawnPromise = new Promise((resolve, reject) => {
    const serverStartedLog = 'Starting JSON-RPC server'
    const serverStartedMsg = 'nexad started'

    const nexadProc = spawn('../e2e/init-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    logger.info(`Starting nexad process... timeout: ${timeout}ms`)
    if (runConfig.verboseLog) {
      nexadProc.stdout.pipe(process.stdout)
    }

    nexadProc.stdout.on('data', (d) => {
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start nexad timeout!')), timeout)
  })
