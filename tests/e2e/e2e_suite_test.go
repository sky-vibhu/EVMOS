...
// runInitialNode builds a docker image capable of running an Nexa node with the given version.
...
func (s *IntegrationTestSuite) runInitialNode(version upgrade.VersionConfig) {
	...
	s.Require().NoError(err, "can't build container with Nexa version: %s", version.ImageTag)
	...
	s.Require().NoError(err, "can't run node with Nexa version: %s", version)
...
// runNodeWithCurrentChanges builds a docker image using the current branch of the Nexa repository.
...
func (s *IntegrationTestSuite) runNodeWithCurrentChanges() {
	...
	err = s.upgradeManager.RunNode(node)
	s.Require().NoError(err, "can't run node Nexa using branch %s", branch)
}
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if Nexa is lower than v10.x.x no need to use the legacy proposal
	currentVersion, err := s.upgradeManager.GetNodeVersion(ctx)
	s.Require().NoError(err, "can't get current Nexa version")
	...
	exec, err := s.upgradeManager.CreateSubmitProposalExec(
		...
		isLegacyProposal,
		"--fees=10000000000000000aNEXB",
		"--gas=500000",
	)
	s.Require().NoErrorf(
		err,
		"can't create the proposal to upgrade Nexa to %s at height %d with name %s",
	...
	s.Require().NoErrorf(
		err,
		"failed to submit proposal to upgrade Nexa to %s at height %d\nstdout: %s,\nstderr: %s",
...
func (s *IntegrationTestSuite) voteForProposal(id int) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	exec, err := s.upgradeManager.CreateVoteProposalExec(s.upgradeParams.ChainID, id, "--fees=10000000000000000aNEXB", "--gas=500000")
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	// export node .nexad to local build/
	...
	node.SetCmd([]string{"nexad", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID), fmt.Sprintf("--home=%s.nexad", rootDir)})
	err = s.upgradeManager.RunNode(node)
