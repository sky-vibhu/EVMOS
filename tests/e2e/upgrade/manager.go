	...
	err = m.pool.Retry(
		func() error {
			...
			if c.State.ExitCode != 0 {
				stdOut, stdErr, _ := m.GetLogs(resource.Container.ID)
				return fmt.Errorf(
					"can't start nexa node, container exit code: %d\n\n[error stream]:\n\n%s\n\n[output stream]:\n\n%s",
					c.State.ExitCode,
					stdErr,
					stdOut,
				)
...
// WaitForHeight queries the Nexa node every second until the node will reach the specified height.
...
// GetNodeHeight calls the Nexa CLI in the current node container to get the current block height
func (m *Manager) GetNodeHeight(ctx context.Context) (int, error) {
	exec, err := m.CreateExec([]string{"nexad", "q", "block"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return 0, fmt.Errorf("nexa query error: %s", errBuff.String())
	}
	return h, nil
}

// GetNodeVersion calls the Nexa CLI in the current node container to get the
// current node version
func (m *Manager) GetNodeVersion(ctx context.Context) (string, error) {
	exec, err := m.CreateExec([]string{"nexad", "version"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return "", fmt.Errorf("nexa version error: %s", errBuff.String())
	}
