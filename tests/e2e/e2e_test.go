...
// TestUpgrade tests if an Nexa node can be upgraded from one version to another.
...
// TestCLITxs executes different types of transactions against an Nexa node
	...
	}{
		{
			...
			cmd: func() (string, error) {
				return s.upgradeManager.CreateSubmitProposalExec(
					...
					5000,
					true,
					"--fees=5000000000aNEXB",
					"--gas-prices=50000aNEXB",
				)
			},
			expPass:   false,
