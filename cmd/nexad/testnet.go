...
import (
	...
	"github.com/nexablock/nexablock/crypto/hd"
	...
	"github.com/nexablock/nexablock/testutil/network"
)
...
Example:
nexad testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "nexad", "Home directory of the node's daemon configuration")
...
Example:
nexad testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("nexa_%d-1", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		kb, err := keyring.New(sdk.KeyringServiceName(), args.keyringBackend, nodeDir, inBuf, clientCtx.Codec, nexakr.Option())
		if err != nil {
			return err
		}
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, nexatypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(cmdcfg.BaseDenom, accStakingTokens),
		}

		genBalances = append(genBalances, banktypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, &nexatypes.EthAccount{
			BaseAccount: authtypes.NewBaseAccount(addr, nil, 0, 0),
			CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
		})

		valTokens := sdk.TokensFromConsensusPower(100, nexatypes.PowerReduction)
