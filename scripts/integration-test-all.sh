...
## default port prefixes for nexad
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/nexad keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/nexad init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/nexad add-genesis-account \
    "$("$PWD"/build/nexad keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000anexama,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/nexad gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/nexad collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/nexad validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/nexad start --pruning=nothing --rpc.unsafe \
    ...
    NEXA_PID=$!
    echo "started nexa node, pid=$NEXA_PID"
    # add PID to array
    arr+=("$NEXA_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test nexa node $HOST_RPC ..."
...
stop_func() {
    NEXA_PID=$i
    echo "shutting down node, pid=$NEXA_PID ..."
