...
const (
	// MainnetChainID defines the Nexa EIP155 chain ID for mainnet
	MainnetChainID = "nexa_9001"
	// TestnetChainID defines the Nexa EIP155 chain ID for testnet
	TestnetChainID = "nexa_9000"
	// BaseDenom defines the Nexa mainnet denomination
	BaseDenom = "aNEXB"
)

// IsMainnet returns true if the chain-id has the Nexa mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Nexa testnet EIP155 chain prefix.
...
// GetNexaAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `nexa`).
// The function fails if the provided bech32 address is invalid.
func GetNexaAddressFromBech32(address string) (sdk.AccAddress, error) {
