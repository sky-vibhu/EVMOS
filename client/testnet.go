...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(nexatypes.AttoNexa, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(nexatypes.AttoNexa, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(nexatypes.AttoNexa)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, nexatypes.AttoNexa, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
